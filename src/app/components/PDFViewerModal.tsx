...
export default function PDFViewerModal({ isOpen, onClose, fileUrl, fileName, documentId, isPDF = true }: PDFViewerModalProps) {
  ...
  useEffect(() => {
    ...
    if (isOpen) { setError(null); setLoading(true); setActiveTab(isPDF ? 'preview' : 'content'); setText(''); setTextError(null); }
  ...
  const fetchDocumentText = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.error) setTextError(data.error);
      else setText(data.fullText || 'No text content available');
    } catch (err) {
          ...
          {isPDF && (
            <div className="flex border-t border-gray-200 dark:border-gray-800">
              {(['preview', 'content'] as const).map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 px-4 py-3 text-sm font-medium transition-colors ${
                  activeTab === tab ? 'text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400 bg-blue-50 dark:bg-blue-900/20' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800'
                }`}>
        ...
        <div className="flex-1 overflow-hidden">
              ...
              {error ? (
                <div className="flex flex-col items-center justify-center h-full p-8">
                  <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-6 max-w-md">
                    <h3 className="text-lg font-semibold text-yellow-800 dark:text-yellow-200 mb-2">PDF File Not Available</h3>
                    <p className="text-yellow-700 dark:text-yellow-300 mb-4">{error}</p>
                    {documentId && (
                      <button onClick={() => setActiveTab('content')} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium">
          ...
          {(!isPDF || activeTab === 'content') && (
            <div className="h-full overflow-auto p-6">
              {textLoading ? (
                <div className="flex items-center justify-center h-full"><p className="text-gray-500 dark:text-gray-400">Loading...</p></div>
              ) : textError ? (
                ...
                <div className="space-y-4">
                  <p className="text-sm text-gray-500 dark:text-gray-400">Formatting may be inconsistent from source.</p>
